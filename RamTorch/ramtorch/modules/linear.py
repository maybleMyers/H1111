...
class CPUBouncingLinear(nn.Module):
    ...
    def _apply(self, fn):
        """
        Override _apply to ensure weights always stay on CPU.
        This prevents .to(device) calls from moving bouncing linear weights to GPU.
        """
        # Apply function to everything except weight and bias parameters
        for name, param in self.named_parameters():
            if name in ['weight', 'bias']:
                continue  # Skip weight and bias - they must stay on CPU
            if param is not None:
                param.data = fn(param.data)
                if param._grad is not None:
                    param._grad.data = fn(param._grad.data)

        # Apply to buffers and other attributes
