...
# Vendored because the deploy environment pins transformers 4.46.x, which predates
# qwen3_vl. Attribute names match the upstream module exactly so the checkpoint's
# vision_encoder/ state dict loads without key rewriting. SDPA-only (no FA2 path),
# inference-only (no gradient checkpointing).
#
# Cosmos3 checkpoints bundle this tower for the native prompt upsampler: the
# understanding-pathway LM consumes its features (with deepstack injection) when
# upsampling an image-to-video prompt against the conditioning frame.
...
def load_vision_encoder(
    ...
    """Load <ckpt_dir>/vision_encoder if present; returns None when the checkpoint ships no tower."""
    subdir = os.path.join(ckpt_dir, "vision_encoder")
    ...
    model = Qwen3VLVisionModel(
        ...
        deepstack_visual_indexes=cfg.get("deepstack_visual_indexes", []),
    )

    shards = sorted(glob.glob(os.path.join(subdir, "*.safetensors")))
    if not shards:
        raise FileNotFoundError(f"no safetensors weights found under {subdir}")
    import safetensors.torch

    state_dict: dict[str, torch.Tensor] = {}
    for shard in shards:
        state_dict.update(safetensors.torch.load_file(shard, device="cpu"))
    # transformers saves the tower either flat or under a `model.` prefix depending on version
    if any(k.startswith("model.") for k in state_dict) and not any(k.startswith("blocks.") for k in state_dict):
        state_dict = {k[len("model.") :]: v for k, v in state_dict.items() if k.startswith("model.")}
    missing, unexpected = model.load_state_dict(state_dict, strict=False)
    if missing:
        raise RuntimeError(f"vision_encoder missing keys: {missing[:8]}{'...' if len(missing) > 8 else ''}")
    if unexpected:
        raise RuntimeError(f"vision_encoder unexpected keys: {unexpected[:8]}{'...' if len(unexpected) > 8 else ''}")
    return model.to(device=device, dtype=dtype).eval()


def load_preprocessor_config(ckpt_dir: str) -> dict:
    """Read the checkpoint's image preprocessing params, with Cosmos3 defaults as fallback."""
    defaults = {
        "patch_size": 16,
    ...
    path = os.path.join(ckpt_dir, "preprocessor_config.json")
