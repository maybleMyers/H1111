...
class Cosmos3AttnProcessor:
    ...
    When ``transfer_info`` is supplied (multi-control video transfer), the generation pathway instead runs N
    independent single-control passes (cosmos-framework ``multi_control_two_way_attention`` parity): in pass *i* the
    queries are the ``ctrl_i`` and noisy-target gen tokens and KV = ``[text-und | ctrl_i | noisy-target]``; the final
    noisy-target output is the control-weighted sum of the per-pass noisy outputs, while each control's own output
    comes from its own pass. ``transfer_info is None`` (or a single vision item) keeps the exact original two-way
    behavior.
    """
    ...
    def __call__(
        ...
        if transfer_info is None:
            ...
            # Multi-control transfer pathway (cosmos-framework mot/attention.py multi_control_two_way_attention).
            # Gen segment layout: [ctrl_1 | ... | ctrl_N | noisy-target]; spans are relative to gen_seq.
            ctrl_spans, (noisy_s, noisy_e), ctrl_weights = transfer_info
            if noisy_e != gen_seq.shape[0]:
                raise ValueError(
                    f"transfer noisy span end ({noisy_e}) must terminate the gen segment ({gen_seq.shape[0]})."
                )
            noisy_q = q_gen[noisy_s:noisy_e]
            noisy_k = k_gen[noisy_s:noisy_e]
            noisy_v = v_gen[noisy_s:noisy_e]
            full_out = q_gen.new_zeros(q_gen.shape[0], attn.num_attention_heads * attn.head_dim)
            noisy_out_acc: torch.Tensor | None = None
            for weight, (cs, ce) in zip(ctrl_weights, ctrl_spans):
                # KV context for this pass: [text-und | ctrl_i | noisy-target]. Query rows are independent under
                # maskless SDPA, so ctrl_i and noisy queries share one call (identical to two separate passes).
                kv_k_i = torch.cat([k_und_for_gen, k_gen[cs:ce], noisy_k], dim=0)
                kv_v_i = torch.cat([v_und, v_gen[cs:ce], noisy_v], dim=0)
                q_i = torch.cat([q_gen[cs:ce], noisy_q], dim=0)
                out_i = dispatch_attention_fn(
                    q_i.unsqueeze(0),
                    kv_k_i.unsqueeze(0),
                    kv_v_i.unsqueeze(0),
                    is_causal=False,
                ...
                out_i = out_i.squeeze(0).flatten(-2, -1)
                n_ctrl = ce - cs
                full_out[cs:ce] = out_i[:n_ctrl]  # ctrl_i output comes from its own pass
                noisy_out_i = out_i[n_ctrl:]
                if noisy_out_acc is None:
                    noisy_out_acc = weight * noisy_out_i
                else:
                    noisy_out_acc = noisy_out_acc + weight * noisy_out_i
...
class Cosmos3OmniTransformer(ModelMixin, ConfigMixin, PeftAdapterMixin, AttentionMixin):
    ...
    def __init__(
        ...
        # offloading (H1111 block swap)
        self.blocks_to_swap = None
        self.offloader = None

    # -------------------------------------------------------------------------
    # H1111 block swap (mirrors wan/modules/model.py WanModel).
    # -------------------------------------------------------------------------
    ...
    def enable_block_swap(self, blocks_to_swap: int, device: torch.device, supports_backward: bool = False):
        ...
        print(
            f"Cosmos3OmniTransformer: Block swap enabled. Swapping {self.blocks_to_swap} blocks out of "
    ...
    def forward(
        ...
        Args:
            ...
            vision_item_spans: Optional per-vision-item ``(start, end)`` token spans relative to the start of the
                generation (post-text) segment, ordered ``[ctrl_1, ..., ctrl_N, noisy-target]``. When more than one
                span is supplied, the layers run multi-control two-way attention (video transfer); with at most one
        ...
        # Multi-control transfer: with >1 vision item the layers run N single-control attention passes; the last
        # span is the noisy target, the preceding ones are the (fully clean) control items. With <=1 item the
        # layers take the original fast path so non-transfer modes are byte-identical.
        transfer_info: tuple[list[tuple[int, int]], tuple[int, int], list[float]] | None = None
        if vision_item_spans is not None and len(vision_item_spans) > 1:
            if control_weights is None or len(control_weights) != len(vision_item_spans) - 1:
                raise ValueError(
                    "`control_weights` must supply one weight per control item: expected "
                    f"{len(vision_item_spans) - 1}, got "
                    f"{None if control_weights is None else len(control_weights)}."
                )
            ctrl_spans = [(int(s), int(e)) for s, e in vision_item_spans[:-1]]
