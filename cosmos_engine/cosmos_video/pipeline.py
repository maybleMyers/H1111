# Vendored from huggingface/diffusers@6afdfd9 for H1111 native Cosmos3 support; modified.
...
def _classify_height_width_bin(height: int, width: int, ratios: dict) -> tuple[int, int]:
    """Return the binned (height, width) whose aspect ratio is closest to the input.

    Local copy of ``VaeImageProcessor.classify_height_width_bin`` from newer diffusers; in the pinned diffusers
    0.33.1 this helper only exists on ``PixArtImageProcessor``, so it is inlined here (verbatim logic).
    """
...
class Cosmos3OmniPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        # Fallback chat template for tokenizers whose checkpoint ships the template as a
        # separate `chat_template.jinja` file (older transformers do not auto-load it).
        # The loader may set this to the template string; it is only consulted when the
        # tokenizer itself has no built-in `chat_template`.
    ...
    @staticmethod
    def _pad_video_temporal(frames: torch.Tensor, target_frames: int) -> torch.Tensor:
        """Pad a ``[B, C, T, H, W]`` tensor along time (cosmos-framework ``pad_temporal_frames`` reflect/repeat)."""
        ...
        padded = frames
        while padded.shape[2] < target_frames:
            pad_len = min(padded.shape[2] - 1, target_frames - padded.shape[2])
            if pad_len <= 0:
                pad_frame = padded[:, :, -1:].expand(-1, -1, target_frames - padded.shape[2], -1, -1)
                padded = torch.cat([padded, pad_frame], dim=2)
                break
            padded = torch.cat([padded, padded.flip(dims=[2])[:, :, :pad_len]], dim=2)
        return padded

    def _prepare_transfer_vision_segments(
        self,
        ...
        """Build the static multi-item vision segment for transfer: ``[ctrl_1 | ... | ctrl_N | noisy-target]``.

        Control items are fully clean (condition mask 1 everywhere: no noisy frames, no timestep embeds, never
        denoised); the target follows normal v2v masking via ``condition_frame_indexes``. With
        ``share_vision_temporal_positions=True`` (framework default) every item reuses the target's mRoPE temporal
        offset (identical per-item position grids); with ``False`` each item continues the running temporal offset
        sequentially, matching the framework SequencePlan packer in both regimes.
        """
        config = self.transformer.config
        latent_patch_size = config.latent_patch_size
        effective_fps = vision_fps if config.enable_fps_modulation else None

        items = list(control_latents) + [target_latents]
        if share_vision_temporal_positions:
            first_shape = tuple(items[0].shape[2:])
            for item_idx, item in enumerate(items[1:], start=1):
                if tuple(item.shape[2:]) != first_shape:
                    raise ValueError(
                        "share_vision_temporal_positions requires equal latent grids across vision items, got "
                        f"item {item_idx} shape {tuple(item.shape[2:])} vs first {first_shape}."
                    )

        token_shapes: list[tuple[int, int, int]] = []
        noisy_frame_indexes_list: list[torch.Tensor] = []
        item_spans: list[tuple[int, int]] = []
        mrope_segments: list[torch.Tensor] = []
        mse_loss_indexes: list[int] = []
        running_offset: int | float = mrope_offset
        seq_pos = curr
        num_noisy_vision_tokens = 0

        for item_idx, latents_item in enumerate(items):
            is_target = item_idx == len(items) - 1
            _, _, latent_t, latent_h, latent_w = latents_item.shape
            patch_h = math.ceil(latent_h / latent_patch_size)
            patch_w = math.ceil(latent_w / latent_patch_size)
            num_item_tokens = latent_t * patch_h * patch_w
            token_shapes.append((latent_t, patch_h, patch_w))

            if is_target:
                cond_frames = {idx for idx in condition_frame_indexes if 0 <= idx < latent_t}
                noisy_frames = [idx for idx in range(latent_t) if idx not in cond_frames]
            else:
                noisy_frames = []  # controls: fully conditioned, never denoised
            noisy_frame_indexes_list.append(torch.tensor(noisy_frames, device=device, dtype=torch.long))

            frame_token_stride = patch_h * patch_w
            for frame_idx in noisy_frames:
                frame_start = seq_pos + frame_idx * frame_token_stride
                mse_loss_indexes.extend(range(frame_start, frame_start + frame_token_stride))
            if is_target:
                num_noisy_vision_tokens = len(noisy_frames) * frame_token_stride

            item_offset = mrope_offset if share_vision_temporal_positions else running_offset
            item_mrope_ids, running_offset = get_3d_mrope_ids_vae_tokens(
                grid_t=latent_t,
                grid_h=patch_h,
                grid_w=patch_w,
                temporal_offset=item_offset,
                reset_spatial_indices=config.unified_3d_mrope_reset_spatial_ids,
                fps=effective_fps,
                base_fps=float(config.base_fps),
                temporal_compression_factor=self.vae_scale_factor_temporal,
            )
            mrope_segments.append(item_mrope_ids.to(device))

            item_spans.append((seq_pos - curr, seq_pos - curr + num_item_tokens))
            seq_pos += num_item_tokens

        num_vision_tokens = seq_pos - curr
        return {
            # Transformer-facing fields (vision_tokens and vision_timesteps spliced per step).
            "vision_token_shapes": token_shapes,
            "vision_sequence_indexes": torch.arange(curr, curr + num_vision_tokens, dtype=torch.long, device=device),
            "vision_mse_loss_indexes": torch.tensor(mse_loss_indexes, dtype=torch.long, device=device),
            "vision_noisy_frame_indexes": noisy_frame_indexes_list,
            "vision_item_spans": item_spans,
            # Assembly helpers (consumed inline before the transformer call).
            "vision_mrope_ids": torch.cat(mrope_segments, dim=1),
            "num_vision_tokens": num_vision_tokens,
    ...
    def _prepare_action_video_conditioning(
        ...
        target_h, target_w = _classify_height_width_bin(
    ...
    def _check_transfer_inputs(
        self,
        control_videos: dict[str, Any],
        control_weights: list[float] | None,
        enable_sound: bool,
        action: "CosmosActionCondition | None",
        num_frames: int | None,
    ) -> tuple[list[str], list[float]]:
        """Validate transfer inputs; return the canonical hint order and normalized control weights."""
        if enable_sound:
            raise ValueError("Video transfer (`control_videos`) cannot be combined with `enable_sound=True`.")
        if action is not None:
            raise ValueError("Video transfer (`control_videos`) cannot be combined with an `action` run.")
        if num_frames is not None and num_frames == 1:
            raise ValueError("Video transfer requires `num_frames` > 1.")
        unknown = [key for key in control_videos if key not in TRANSFER_HINTS]
        if unknown:
            raise ValueError(f"Unknown transfer hint(s) {unknown}; expected keys from {list(TRANSFER_HINTS)}.")
        if not control_videos:
            raise ValueError("`control_videos` must contain at least one hint.")
        supplied = list(control_videos.keys())
        if control_weights is None:
            weights = [1.0] * len(supplied)
        else:
            if len(control_weights) != len(supplied):
                raise ValueError(
                    f"`control_weights` length ({len(control_weights)}) must match the number of supplied hints "
                    f"({len(supplied)})."
                )
            weights = [float(w) for w in control_weights]
        ...
        weight_by_hint = dict(zip(supplied, weights))
        # Canonical (training) hint order: edge/blur/depth/seg/wsm.
        ordered_hints = [hint for hint in TRANSFER_HINTS if hint in weight_by_hint]
        return ordered_hints, [weight_by_hint[hint] / total for hint in ordered_hints]
    ...
    def tokenize_prompt(
        ...
        def _tokenize(text: str) -> BatchEncoding:
            ...
            # `chat_template.jinja` file; when the tokenizer has no built-in template,
            # fall back to the template string supplied by the loader (if any).
            extra_kwargs: dict[str, Any] = {}
            if getattr(self.text_tokenizer, "chat_template", None) is None:
                fallback_template = getattr(self, "_chat_template", None)
                if fallback_template is not None:
                    extra_kwargs["chat_template"] = fallback_template
    ...
    def __call__(
        ...
        condition_frame_indexes_vision: Iterable[int] = (0, 1),
        condition_video_keep: Literal["first", "last"] = "first",
        control_videos: "dict[str, list] | None" = None,
        ...
        Args:
            ...
            control_videos (`dict[str, list]`, *optional*):
                Video-transfer control hints: a mapping from hint name (`"edge"`, `"blur"`, `"depth"`, `"seg"`,
                `"wsm"`) to the hint's control frames (any format `video` accepts). Each hint is VAE-encoded and
                packed as an extra fully clean vision item — sequence `[text | ctrl_1 | ... | ctrl_N | noisy
                target]` — and attended via multi-control two-way attention. Composes with `video` /
                `condition_frame_indexes_vision` conditioning (used for chunked transfer); mutually exclusive with
                `enable_sound=True` and `action`.
            control_weights (`list[float]`, *optional*):
                Per-hint attention weights (one per supplied hint, default equal). Normalized to sum to 1.
            control_guidance (`float`, *optional*, defaults to `1.0`):
                Control-CFG scale. When != 1.0, each step runs an extra no-control forward pass and the conditional
                target velocity becomes `v_nc + control_guidance * (v_full - v_nc)` before the standard text-CFG
                combine. `1.0` adds no extra forward pass.
            control_guidance_interval (`tuple[float, float]`, *optional*):
                `(lo, hi)` gate for control-CFG on the 0-1000 timestep scale (strict bounds, framework parity).
            share_vision_temporal_positions (`bool`, *optional*, defaults to `True`):
                When `True` control items share the target's mRoPE temporal position ids; when `False` each vision
                item gets its own sequential temporal offsets. Must match the trained checkpoint's regime.
        ...
        # Transfer (multi-control) validation + canonical hint ordering / weight normalization.
        is_transfer = control_videos is not None and len(control_videos) > 0
        transfer_hint_order: list[str] = []
        transfer_weights: list[float] = []
        if control_videos is not None:
            transfer_hint_order, transfer_weights = self._check_transfer_inputs(
                control_videos, control_weights, enable_sound, action, num_frames
            )
        ...
        if action is not None:
            ...
            height, width = _classify_height_width_bin(
                source_h, source_w, ratios=_ACTION_RESOLUTION_BINS[resolution_key]
            )
        ...
        control_latents: list[torch.Tensor] = []
        if is_transfer:
            for hint in transfer_hint_order:
                control_tensor = self.video_processor.preprocess_video(
                    control_videos[hint], height=height, width=width
        ...
        # once and reused each step; control_guidance == 1.0 stays zero-overhead (no extra state, no extra forward).
        nc_packed_static: dict[str, Any] | None = None
        if is_transfer and control_guidance != 1.0:
            nc_vision_segment = self._prepare_vision_segment(
                input_vision_tokens=latents,
                has_image_condition=has_image_condition,
                mrope_offset=cond_text_segment["vision_start_temporal_offset"],
                vision_fps=fps_vision,
            ...
            nc_packed_static = {
                ...
                **nc_vision_segment,
                "position_ids": torch.cat(
                    [cond_text_segment["text_mrope_ids"], nc_vision_segment["vision_mrope_ids"]], dim=1
                ),
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # --- Control-CFG (transfer only; framework transfer.py:293) ---
                # v_cond <- v_nc + control_guidance * (v_full - v_nc), optionally gated by a strict (lo, hi)
                # interval on the 0-1000 timestep scale. Mixed into the COND branch before the text-CFG combine.
                if nc_packed_static is not None and (
                    control_guidance_interval is None
                    or control_guidance_interval[0] < float(t) < control_guidance_interval[1]
                ):
                    preds_vision_nc, _, _ = self.transformer(
                        input_ids=nc_packed_static["input_ids"],
                        text_indexes=nc_packed_static["text_indexes"],
                        position_ids=nc_packed_static["position_ids"],
                        und_len=nc_packed_static["und_len"],
                        sequence_length=nc_packed_static["sequence_length"],
                        vision_tokens=[vision_tokens],
                        vision_token_shapes=nc_packed_static["vision_token_shapes"],
                        vision_sequence_indexes=nc_packed_static["vision_sequence_indexes"],
                    ...
                    cond_v_vision = nc_v_vision + control_guidance * (cond_v_vision - nc_v_vision)
                ...
                def _combine_cfg(uncond_v, cond_v):
                    # guidance_interval gates CFG on the 0-1000 timestep scale;
                    # normalize_cfg rescales the guided velocity to the cond norm
                    # (cosmos-framework omni_mot_model.velocity_fn parity).
                    if uncond_v is None or not self.do_classifier_free_guidance:
                        return cond_v
                    ...
                    if normalize_cfg:
                        v = v * (cond_v.norm() / (v.norm() + 1e-8)).clamp(max=1.0)
                    return v

                velocity_vision = _combine_cfg(uncond_v_vision, cond_v_vision)
                ...
                if action_scheduler is not None and has_noisy_action and cond_v_action is not None:
                    velocity_action = _combine_cfg(uncond_v_action, cond_v_action)
