...
# Note: filter_lora_state_dict is from local utils, not wan.vace
...
# Pusa components - only keep utilities, remove conflicting schedulers
try:
    ...
    PUSA_UTILS_AVAILABLE = True
except ImportError:
    PUSA_UTILS_AVAILABLE = False
    print("Pusa utilities not available. Some conditioning features may be limited.")
...
try:
    ...
    from wan.vace.utils import (
        ...
        save_videos_grid,
        filter_kwargs,  # For scheduler configuration
        merge_lora,
        unmerge_lora,
    ...
    # Scheduler imports
    from wan.vace.utils.fm_solvers import FlowDPMSolverMultistepScheduler
    from wan.vace.utils.fm_solvers_unipc import FlowUniPCMultistepScheduler
    from diffusers import FlowMatchEulerDiscreteScheduler
    from omegaconf import OmegaConf
    VACE_AVAILABLE = True
except ImportError as e:
    VACE_AVAILABLE = False
    print(f"VACE modules not available. Error: {e}")
...
def parse_args() -> argparse.Namespace:
    ...
    # Adjust available solvers based on VACE availability
    solver_choices = ["unipc", "dpm++", "vanilla"]
    if not VACE_AVAILABLE:  # Only add Pusa when VACE is not available to avoid conflicts
        solver_choices.append("pusa")
    
    parser.add_argument(
        "--sample_solver", type=str, default="unipc", choices=solver_choices, 
        help="The solver used to sample. Note: 'pusa' unavailable when VACE models detected."
    )
    ...
    # Pusa Extension arguments (only when VACE not available)
    ...
    parser.add_argument("--vace_config", type=str, default=None, 
                       help="Path to VACE configuration YAML file")
    parser.add_argument("--subject_ref_images", type=str, nargs="+", default=None,
                       help="Reference images for subject preservation (S2V mode)")
    parser.add_argument("--vace_context_scale", type=float, default=1.0,
                       help="VACE context influence strength (0.0-2.0, default: 1.0)")
    parser.add_argument("--control_video", type=str, default=None,
                       help="Control video for V2V generation with VACE")
    
    # VACE Optimization arguments
    parser.add_argument("--enable_teacache", action="store_true",
                       help="Enable TeaCache acceleration for VACE models")
    parser.add_argument("--teacache_threshold", type=float, default=0.10,
                       help="TeaCache threshold (0.05-0.30, default: 0.10)")
    parser.add_argument("--num_skip_start_steps", type=int, default=5,
                       help="Number of steps to skip TeaCache at start (default: 5)")
    ...
    parser.add_argument("--cfg_skip_ratio", type=float, default=0.0,
                       help="CFG skip ratio for acceleration (0.0-0.25, default: 0.0)")
    parser.add_argument("--enable_riflex", action="store_true",
                       help="Enable RiFlex technology for improved temporal consistency")
    parser.add_argument("--riflex_k", type=int, default=6,
                       help="Index of intrinsic frequency for RiFlex (default: 6)")
    parser.add_argument("--padding_in_subject_ref_images", action="store_true",
                       help="Add padding to reference images to match aspect ratio")
    parser.add_argument("--vace_generation_mode", type=str, default="auto",
                       choices=["auto", "t2v", "i2v", "s2v", "v2v", "flf", "camera"],
                       help="VACE generation mode (auto=detect from inputs)")
...
class DynamicModelManager:
        ...
        try:
            with safe_open(model_path, framework="pt", device="cpu") as f:
                ...
                vace_keys = [
                    ...
                    "vace_blocks",
                    "blocks.0.before_proj.weight",
                    "blocks.0.after_proj.weight"
                ]
                return any(key in keys or any(vace_key in key for key in keys) for vace_key in vace_keys)
        except Exception as e:
            logger.warning(f"Error detecting VACE model: {e}")
            return False
    
    def load_vace_config(self, config_path: str = None):
        """Load VACE configuration from YAML file."""
        if not VACE_AVAILABLE:
            return None
            
        if config_path is None:
            # Try multiple default VACE config paths based on task
            task_to_config = {
                "t2v": "wan_civitai_t2v.yaml",
                "i2v": "wan_civitai_i2v.yaml",
                "s2v": "wan_civitai_s2v.yaml",
                "v2v": "wan_civitai_v2v.yaml"
            }
            
            # Determine config based on task
            for task_key, config_file in task_to_config.items():
                if task_key in self.args.task.lower():
                    config_paths = [
                        f"VideoX-Fun/config/wan2.2/{config_file}",
                        f"config/wan2.2/{config_file}",
                        f"VideoX-Fun/examples/wan2.2_fun/config/{config_file}"
                    ]
                    break
            else:
                # Default config paths
                config_paths = [
                    "VideoX-Fun/config/wan2.2/wan_civitai_t2v.yaml",
        ...
        for path in config_paths:
            try:
                if os.path.exists(path):
                    self.vace_config = OmegaConf.load(path)
                    logger.info(f"Loaded VACE config from: {path}")
                    return self.vace_config
            except Exception as e:
                logger.debug(f"Could not load config from {path}: {e}")
                continue
                
        logger.warning(f"VACE config not found in any of the paths: {config_paths}")
        # Return default config
        return self._get_default_vace_config()
    
    def _get_default_vace_config(self):
        """Get default VACE configuration."""
    ...
    def _load_vace_model(self, model_type: str, loading_device: str, loading_weight_dtype, 
                        lora_weights_list=None, lora_multipliers=None):
        """Load VACE model using VideoX-Fun pipeline with proper from_pretrained method."""
        if not VACE_AVAILABLE:
            raise RuntimeError("VACE models not available. Install VideoX-Fun dependencies.")
            
        model_path = self.model_paths[model_type]
        logger.info(f"Loading VACE {model_type} noise model from: {model_path}")
        
        try:
            # Load configuration if available
            if self.vace_config is not None:
                transformer_kwargs = OmegaConf.to_container(
                    self.vace_config.get('transformer_additional_kwargs', {})
                )
            else:
            ...
            if lora_weights_list is not None and VACE_AVAILABLE:
                for i, lora_sd in enumerate(lora_weights_list):
                    ...
                    model = merge_lora(model, lora_sd, multiplier, device=loading_device)
    ...
    def set_model_paths(self, low_path: str, high_path: str):
        ...
        # Detect if models are VACE variants
        self.is_vace_model = self.detect_vace_model(low_path)
        if self.is_vace_model:
            logger.info("Detected VACE model - enabling VACE functionality")
            self.load_vace_config()
...
def load_vae(args: argparse.Namespace, config, device: torch.device, dtype: torch.dtype, vace_config=None):
    ...
    Args:
        ...
        vace_config: VACE configuration from OmegaConf (optional)

    Returns:
        WanVAE, Wan2_2_VAE, or VACE VAE: loaded VAE model
    """
    # Check if using VACE models
    if VACE_AVAILABLE and vace_config is not None:
        # Use VACE VAE loading
        vae_type = vace_config.get('vae_kwargs', {}).get('vae_type', 'AutoencoderKLWan')
        vae_subpath = vace_config.get('vae_kwargs', {}).get('vae_subpath', 'vae')
        
        logger.info(f"Loading VACE VAE type: {vae_type}")
        
        # Select appropriate VAE class
        Chosen_AutoencoderKL = {
            "AutoencoderKLWan": AutoencoderKLWan,
            "AutoencoderKLWan3_8": AutoencoderKLWan3_8
        }.get(vae_type, AutoencoderKLWan)
        
        # Determine VAE path
        if args.vae and os.path.exists(args.vae):
            vae_path = args.vae
        elif args.dit and os.path.isdir(args.dit):
            # Try to find VAE in model directory
            vae_path = os.path.join(args.dit, vae_subpath)
        else:
            # Fallback to standard path
            vae_path = os.path.join("models", "Diffusion_Transformer", vae_subpath)
        
        if os.path.isdir(vae_path):
            # Load from directory using from_pretrained
            vae = Chosen_AutoencoderKL.from_pretrained(
                vae_path,
                additional_kwargs=OmegaConf.to_container(vace_config.get('vae_kwargs', {}))
            ).to(dtype)
        else:
            # Load from file - fallback to standard loading
            logger.warning(f"VAE path {vae_path} is not a directory, falling back to standard loading")
            # Continue with standard loading below
        
        if 'vae' in locals():
            return vae
    
    # Standard VAE loading (non-VACE or fallback)
...
def load_text_encoder(args: argparse.Namespace, config, device: torch.device, vace_config=None):
    ...
    if VACE_AVAILABLE and vace_config is not None:
        try:
            text_encoder_subpath = vace_config.get('text_encoder_kwargs', {}).get('text_encoder_subpath', 'text_encoder')
            ...
            logger.warning(f"Could not load VACE text encoder: {e}, falling back to standard")
    
    # Standard text encoder loading
    ...
    if isinstance(t5_dtype, str):
        t5_dtype = str_to_dtype(t5_dtype)
...
def load_clip_model(args: argparse.Namespace, vace_config=None) -> Optional[object]:
    """Load CLIP model for image conditioning if using VACE I2V/S2V modes.
    
    Args:
        args: command line arguments
        vace_config: VACE configuration
        
    Returns:
        CLIPModel or None
    """
    if not VACE_AVAILABLE or vace_config is None:
        return None
        
    # Only load CLIP for I2V/S2V tasks
    if not any(mode in args.task.lower() for mode in ['i2v', 's2v']):
        return None
        
    try:
        clip_subpath = vace_config.get('clip_kwargs', {}).get('clip_subpath', 'clip')
        
        if args.clip and os.path.exists(args.clip):
            clip_path = args.clip
        elif args.dit and os.path.isdir(args.dit):
            clip_path = os.path.join(args.dit, clip_subpath)
        else:
            logger.warning("No CLIP model path found")
            return None
...
def load_tokenizer(args: argparse.Namespace, config, vace_config=None):
    ...
    if VACE_AVAILABLE and vace_config is not None:
        try:
            tokenizer_subpath = vace_config.get('text_encoder_kwargs', {}).get('tokenizer_subpath', 'tokenizer')
            
            if args.dit and os.path.isdir(args.dit):
                tokenizer_path = os.path.join(args.dit, tokenizer_subpath)
                if os.path.exists(tokenizer_path):
                    tokenizer = AutoTokenizer.from_pretrained(tokenizer_path)
                    logger.info(f"Loaded VACE tokenizer from {tokenizer_path}")
                    return tokenizer
        except Exception as e:
            logger.warning(f"Could not load VACE tokenizer: {e}, using default")
    
    # Fallback to default tokenizer
    return None  # Will be created by text encoder


# VACE Context Processing Functions
def process_vace_subject_references(args: argparse.Namespace, sample_size: Tuple[int, int]) -> Optional[torch.Tensor]:
...
def process_vace_control_video(args: argparse.Namespace, video_length: int, sample_size: Tuple[int, int]) -> Optional[torch.Tensor]:
...
def vace_encode_masks(masks: Optional[torch.Tensor], ref_images: Optional[torch.Tensor],
    ...
    for i in range(masks.shape[0]):
        ...
        # This is critical for the view operation to work correctly
        adjusted_height = (height // vae_stride[1]) * vae_stride[1]
        adjusted_width = (width // vae_stride[2]) * vae_stride[2]

        # Calculate the downsampled dimensions
        new_height = adjusted_height // vae_stride[1]
        new_width = adjusted_width // vae_stride[2]

        # First extract single channel mask
        mask = mask[0, :, :, :]  # [depth, height_orig, width_orig]

        # Resize mask to the adjusted dimensions that are divisible by vae_stride
        # This ensures the subsequent view operation will work
        mask = F.interpolate(
            mask.unsqueeze(0).unsqueeze(0),  # Add batch and channel dims
            size=(depth, adjusted_height, adjusted_width),
            mode='trilinear',
            align_corners=False
        )[0, 0]  # Remove batch and channel dims [depth, adjusted_height, adjusted_width]

        # Now reshape following the VACE approach (with typo fix)
        # Reshape spatial dimensions to create multiple channels from spatial blocks
        mask = mask.view(
...
def detect_vace_generation_mode(args: argparse.Namespace) -> str:
    ...
    if args.subject_ref_images:
        ...
        if args.video_path:  # Both start and end image
            return "flf"  # First-Last-Frame
        else:
            return "i2v"  # Image-to-Video
    else:
        return "t2v"  # Text-to-Video

def create_vace_pipeline(args: argparse.Namespace, transformer, transformer_2, vae, tokenizer, text_encoder, vace_config=None) -> Optional[object]:
    """Create VACE pipeline using Wan2_2FunInpaintPipeline."""
    if not VACE_AVAILABLE:
        return None
        
    try:
        # Get scheduler configuration from VACE config
        scheduler_kwargs = {}
        if vace_config:
            scheduler_kwargs = OmegaConf.to_container(vace_config.get('scheduler_kwargs', {}))
        
        # Load scheduler based on solver type with proper configuration
        if args.sample_solver == "unipc":
            # UniPC needs shift=1 when enabled
            scheduler_kwargs['shift'] = 1
            scheduler = FlowUniPCMultistepScheduler(
                **filter_kwargs(FlowUniPCMultistepScheduler, scheduler_kwargs)
            )
        elif args.sample_solver == "dpm++":
            # DPM++ also needs shift=1 when enabled
            scheduler_kwargs['shift'] = 1
            scheduler = FlowDPMSolverMultistepScheduler(
                **filter_kwargs(FlowDPMSolverMultistepScheduler, scheduler_kwargs)
            )
        else:  # vanilla flow
            # Standard flow uses default shift (usually 12.0)
            if args.flow_shift is not None:
                scheduler_kwargs['shift'] = args.flow_shift
            scheduler = FlowMatchEulerDiscreteScheduler(
                **filter_kwargs(FlowMatchEulerDiscreteScheduler, scheduler_kwargs)
            )
        
        # Create proper VACE pipeline using Wan2_2FunInpaintPipeline
        ...
        logger.info("Created VACE pipeline using Wan2_2FunInpaintPipeline")
        return pipeline
    except Exception as e:
        logger.error(f"Error creating VACE pipeline: {e}")
        # Try fallback to Wan2_2VaceFunPipeline if available
        try:
            logger.info("Attempting fallback to Wan2_2VaceFunPipeline")
...
def enable_vace_optimizations(pipeline, args: argparse.Namespace, model_manager: DynamicModelManager, 
                             num_inference_steps: int, video_length: int, vae_config=None):
    """Enable VACE-specific performance optimizations."""
    if not VACE_AVAILABLE or not model_manager.is_vace_model:
        return
    
    try:
        # Get model name for TeaCache coefficients
        model_name = "models/Diffusion_Transformer/Wan2.2-Fun-A14B-InP"  # Default
        if args.dit and os.path.isdir(args.dit):
            model_name = args.dit
        ...
        if hasattr(args, 'enable_teacache') and args.enable_teacache:
            ...
            if coefficients is not None:
                threshold = getattr(args, 'teacache_threshold', 0.10)
                skip_steps = getattr(args, 'num_skip_start_steps', 5)
                offload = getattr(args, 'teacache_offload', False)
                
                logger.info(f"Enabling TeaCache with threshold {threshold}, skip {skip_steps} steps")
                if hasattr(pipeline.transformer, 'enable_teacache'):
                    pipeline.transformer.enable_teacache(
                        coefficients, 
                        num_inference_steps,
                        threshold,
                        num_skip_start_steps=skip_steps,
                        offload=offload
                    )
                if pipeline.transformer_2 is not None and hasattr(pipeline.transformer_2, 'share_teacache'):
                    pipeline.transformer_2.share_teacache(transformer=pipeline.transformer)
        
        # Enable CFG skip if requested
        cfg_skip_ratio = getattr(args, 'cfg_skip_ratio', 0.0)
        if cfg_skip_ratio > 0.0:
            logger.info(f"Enabling CFG skip ratio: {cfg_skip_ratio}")
            if hasattr(pipeline.transformer, 'enable_cfg_skip'):
                pipeline.transformer.enable_cfg_skip(cfg_skip_ratio, num_inference_steps)
            if pipeline.transformer_2 is not None and hasattr(pipeline.transformer_2, 'share_cfg_skip'):
                pipeline.transformer_2.share_cfg_skip(transformer=pipeline.transformer)
        
        # Enable RiFlex if requested
        if hasattr(args, 'enable_riflex') and args.enable_riflex:
            riflex_k = getattr(args, 'riflex_k', 6)
            # Calculate latent frames based on VAE compression ratio
            temporal_compression = 4  # Default
            if vae_config:
                temporal_compression = vae_config.get('vae_kwargs', {}).get('temporal_compression_ratio', 4)
            
            latent_frames = (video_length - 1) // temporal_compression + 1
            logger.info(f"Enabling RiFlex with k={riflex_k}, L_test={latent_frames}")
            
            if hasattr(pipeline.transformer, 'enable_riflex'):
                pipeline.transformer.enable_riflex(k=riflex_k, L_test=latent_frames)
            if pipeline.transformer_2 is not None and hasattr(pipeline.transformer_2, 'enable_riflex'):
                pipeline.transformer_2.enable_riflex(k=riflex_k, L_test=latent_frames)
        
        # Enable memory optimizations for VACE models
        if args.blocks_to_swap > 0:
            logger.info(f"Block swapping enabled with {args.blocks_to_swap} blocks")
            # Block swapping is already handled by the main pipeline
            
        # Apply FP8 optimization if requested for VACE
        if args.fp8 and hasattr(pipeline.transformer, 'to'):
            logger.info("Applying FP8 optimization to VACE transformers")
            if VACE_AVAILABLE:
                # Use VACE FP8 optimization
                convert_model_weight_to_float8(
        ...
        logger.warning(f"Error enabling VACE optimizations: {e}")

def generate_with_vace_pipeline(args: argparse.Namespace, vace_mode: str, device: torch.device, 
                               cfg, dit_dtype: torch.dtype, dit_weight_dtype: torch.dtype) -> Optional[torch.Tensor]:
    """Generate video using VACE pipeline."""
    if not VACE_AVAILABLE:
        logger.error("VACE pipeline requested but VideoX-Fun not available")
        return None
    
    try:
        logger.info(f"Starting VACE generation with mode: {vace_mode}")
        
        # Load VACE configuration
        if args.vace_config:
            vace_config = OmegaConf.load(args.vace_config)
        else:
            # Use default VACE config
            vace_config_path = "VideoX-Fun/config/wan2.2/wan_civitai_t2v.yaml"
            if os.path.exists(vace_config_path):
                vace_config = OmegaConf.load(vace_config_path)
            else:
                logger.warning("Default VACE config not found, using minimal config")
        ...
        vae_path = args.vae if args.vae else "models/vae/Wan2.1_VAE.pth"  # Default path
        if os.path.exists(vae_path):
            vae = ChosenVAE.from_pretrained(vae_path).to(dit_weight_dtype or dit_dtype)
        else:
        ...
        # Load VACE transformers (dual model support)
        model_path_low = args.dit_low_noise or args.dit
        model_path_high = args.dit_high_noise
        
        if not model_path_low:
            logger.error("No VACE model path specified")
            return None
        
        # Create model manager for VACE models
        ...
        sample_size = (args.video_size[0], args.video_size[1])
        video_length = args.video_length or 49
        
        # Process inputs based on VACE mode
        ...
        if vace_mode == "s2v":
            subject_ref_images = process_vace_subject_references(args, sample_size)
        elif vace_mode == "v2v":
            # V2V can use control video AND optionally start/end images and subject references
            control_video = process_vace_control_video(args, video_length, sample_size)
            
            # Optional: Process subject references for V2V
            if args.subject_ref_images:
                subject_ref_images = process_vace_subject_references(args, sample_size)
            
            # Optional: Process start/end images for V2V (for additional conditioning)
            if args.image_path or args.end_image_path:
                start_image = args.image_path
                end_image = args.end_image_path
                inpaint_video, inpaint_video_mask, _ = get_image_to_video_latent(
                    start_image, end_image, video_length=video_length, sample_size=sample_size
                )
        elif vace_mode == "i2v" or vace_mode == "flf":
            # Process start/end images for I2V or FLF
            start_image = args.image_path[0] if args.image_path else None
            end_image = args.image_path[1] if args.image_path and len(args.image_path) > 1 else None
            inpaint_video, inpaint_video_mask, _ = get_image_to_video_latent(
                start_image, end_image, video_length=video_length, sample_size=sample_size
            )
        ...
        generator = torch.Generator(device=device).manual_seed(args.seed or 42)
        ...
        result = pipeline(
            ...
            boundary=vace_config.get('transformer_additional_kwargs', {}).get('boundary', 0.875),
            shift=args.flow_shift or 12.0,
        )
        
        # Clean up models
        model_manager.cleanup()
        
        # Convert result to expected format [B, C, F, H, W]
        ...
        if isinstance(videos, torch.Tensor):
            if videos.dim() == 4:  # [F, H, W, C]
                videos = videos.permute(3, 0, 1, 2).unsqueeze(0)  # [1, C, F, H, W]
            elif videos.dim() == 5 and videos.shape[0] == 1:  # Already [1, C, F, H, W]
                pass
            else:
                logger.warning(f"Unexpected video tensor shape: {videos.shape}")
                
        logger.info("VACE generation completed successfully")
        return videos
        
    except Exception as e:
        logger.error(f"Error in VACE generation: {e}")
        return None
...
def setup_scheduler(args: argparse.Namespace, config, device: torch.device) -> Tuple[Any, torch.Tensor]:
    ...
    if args.sample_solver == "unipc":
        ...
        try:
            ...
            logger.warning("Pusa schedulers not available, falling back to UniPC")
            scheduler = FlowUniPCMultistepScheduler(
                num_train_timesteps=config.num_train_timesteps,
                shift=args.flow_shift
            )
...
def generate(args: argparse.Namespace) -> Optional[torch.Tensor]:
    ...
    # --- Check for VACE Model ---
    vace_mode = None
    if VACE_AVAILABLE and (args.dit or args.dit_low_noise):
        # Check if VACE model is being used
        model_path = args.dit or args.dit_low_noise
        temp_manager = DynamicModelManager(cfg, device, torch.float16, torch.float16, args)
        if temp_manager.detect_vace_model(model_path):
            logger.info("VACE model detected - enabling VACE generation pipeline")
            vace_mode = detect_vace_generation_mode(args)
            logger.info(f"VACE generation mode: {vace_mode}")
        del temp_manager
...
def main():
    ...
    if not latents_mode:
        ...
        if args.sample_solver == "pusa" and (hasattr(args, 'cond_images') and args.cond_images or 
                                           hasattr(args, 'cond_video') and args.cond_video or 
                                           hasattr(args, 'end_image') and args.end_image):
