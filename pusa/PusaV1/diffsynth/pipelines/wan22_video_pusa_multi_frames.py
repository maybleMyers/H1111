...
class Wan22VideoPusaMultiFramesPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.float16, tokenizer_path=None):
        ...
        self.dit2: WanModelPusa = None
        self.vae: WanVideoVAE = None
        self.motion_controller: WanMotionControllerModel = None
        self.vace: VaceWanModel = None
        self.model_names = ['text_encoder', 'dit', 'dit2', 'vae', 'image_encoder', 'motion_controller', 'vace']
    ...
    def from_model_manager(model_manager: ModelManager, torch_dtype=None, device=None, use_usp=False):
        ...
        pipe = Wan22VideoPusaMultiFramesPipeline(device=device, torch_dtype=torch_dtype)
        ...
        if use_usp:
            ...
            if pipe.dit2 is not None:
                for block in pipe.dit2.blocks:
                    block.self_attn.forward = types.MethodType(usp_attn_forward, block.self_attn)
                pipe.dit2.forward = types.MethodType(usp_dit_forward, pipe.dit2)
    ...
    def __call__(
        ...
        for progress_id, timestep_val in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            if visualize_attention:
                from ..models.wan_video_pusa import _VISUALIZE_ATTENTION_CONFIG
            ...
            for latent_idx in cond_frame_latent_indices:
                ...
                # if timestep_val.item() > switch_DiT_boundary * self.scheduler.num_train_timesteps:
                #     timestep[:,latent_idx] = 0 # high noise model is not good at handling small noise levels
                # else:
            ...
            noise_pred_posi = model_fn_wan_video(
                denoising_models["dit"], motion_controller=self.motion_controller, vace=self.vace,
