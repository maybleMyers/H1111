...
def sample_model(device, dit, encoder, condition_image, condition_frame_idx, conditioning, **args):
    ...
    # We have multiple conditioning frames
    for i, frame_tensor in enumerate(condition_image):
        pos = condition_frame_idx[i]
        z_0[:, :, pos, :, :] = frame_tensor.to(device)
    ...
    # Get noise multipliers if provided
    noise_multipliers = args.get("noise_multiplier", None)
    ...
    for i in get_new_progress_bar(range(0, sample_steps), desc="Sampling"):
        ...
        # Apply conditioning frame noise multipliers
        for j, pos in enumerate(condition_frame_idx):
            # Use individual multipliers if available, otherwise use default
            multiplier = noise_multipliers[j] if isinstance(noise_multipliers, list) and j < len(noise_multipliers) else 0.3
            sigma[:, pos] = sigma[:, pos] * multiplier
        ...
        # Apply noise multipliers to dsigma as well
        for j, pos in enumerate(condition_frame_idx):
            multiplier = noise_multipliers[j] if isinstance(noise_multipliers, list) and j < len(noise_multipliers) else 0.3
            dsigma[:, pos] = sigma[:, pos] - sigma_schedule[i + 1] * multiplier
