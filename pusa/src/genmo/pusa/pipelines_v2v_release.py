...
def sample_model(device, dit, encoder, condition_image, condition_frame_idx, conditioning, **args):
    ...
    for i in get_new_progress_bar(range(0, sample_steps), desc="Sampling"):
        ...
        for idx, pos in enumerate(condition_frame_idx):
            sigma[:, pos] = sigma[:, pos] * float(eval(noise_multiplier)[idx])
        ...
        for idx, pos in enumerate(condition_frame_idx):
            dsigma[:, pos] = sigma[:, pos] - sigma_schedule[i + 1] * float(eval(noise_multiplier)[idx])
