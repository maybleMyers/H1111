...
def generate(args: argparse.Namespace) -> Optional[torch.Tensor]:
    ...
    # Route to LongCat generation if task is longcat
    if args.task.startswith('longcat-'):
...
def decode_latent(latent: torch.Tensor, args: argparse.Namespace, cfg) -> torch.Tensor:
    ...
    vae = None
    is_longcat = hasattr(args, "_longcat_vae") and args._longcat_vae is not None

    if is_longcat:
        vae = args._longcat_vae
        logger.info("Using LongCat VAE instance for decoding.")
