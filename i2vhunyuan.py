...
def create_parameter_transfer_map(metadata: Dict, target_tab: str) -> Dict:
    ...
    mapping = {
        'common': {
            ...
            'negative_prompt': ('i2v_negative_prompt', 'v2v_negative_prompt'),
            'clip_vision_path': ('i2v_clip_vision_path', None),
            'i2v_stability': ('i2v_stability', None),
            'fp8_fast': ('i2v_fp8_fast', None)
        },
        'lora': {
        ...
        with gr.Tab(label="Hunyuan-i2v") as i2v_tab:
            with gr.Row():
                with gr.Column(scale=4):
                    ...
                    i2v_negative_prompt = gr.Textbox(label="Negative Prompt", value="", lines=2, info="Negative prompt")
            ...
            with gr.Row():
                ...
                i2v_te1 = gr.Textbox(label="te1", value="hunyuan/llava-llama-3-8b-text-encoder-tokenizer")
                i2v_te2 = gr.Textbox(label="te2", value="hunyuan/clip_l.safetensors")
                i2v_clip_vision_path = gr.Textbox(label="CLIP Vision Path", value="hunyuan/clip-vit-large-patch14", info="Path to CLIP vision model for HunyuanI2V")
            ...
            with gr.Row():
                ...
                i2v_split_uncond = gr.Checkbox(label="Split Unconditional", value=True, visible=True)
            with gr.Row():
                i2v_stability = gr.Checkbox(label="I2V Stability", value=False, info="Enable stability mode for HunyuanI2V")
                i2v_fp8_fast = gr.Checkbox(label="FP8 Fast", value=False, info="Enable fast FP8 arithmetic (RTX 4XXX+)")
                i2v_compile = gr.Checkbox(label="Compile Model", value=False, info="Enable torch.compile for potentially faster generation")
                i2v_compile_backend = gr.Dropdown(label="Compile Backend", choices=["inductor", "cudagraphs", "onnxrt", "nvfuser"], value="inductor", info="Torch compile backend")
                i2v_compile_mode = gr.Dropdown(label="Compile Mode", choices=["default", "reduce-overhead", "max-autotune", "max-autotune-no-cudagraphs"], value="max-autotune-no-cudagraphs", info="Torch compile mode")
                i2v_compile_dynamic = gr.Checkbox(label="Dynamic Shapes", value=False, info="Use dynamic shapes in compilation")
                i2v_compile_fullgraph = gr.Checkbox(label="Full Graph", value=False, info="Use full graph compilation")
        # Video to Video Tab
...
### SKYREELS
    ...
    i2v_generate_btn.click(
        ...
        inputs=[
            ...
            *i2v_lora_weights, *i2v_lora_multipliers, i2v_input, i2v_strength, i2v_negative_prompt,
            i2v_cfg_scale, i2v_split_uncond, i2v_use_fp8, i2v_clip_vision_path, i2v_stability,
            i2v_fp8_fast, i2v_compile, i2v_compile_backend, i2v_compile_mode, 
            i2v_compile_dynamic, i2v_compile_fullgraph
        ],
