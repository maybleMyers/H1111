...
def extract_specific_frames(video_path, frame_id):
    """Extract a specific frame from video or return image if not video"""
    if is_video(video_path):
        try:
            ...
            try:
                ...
                # Fallback if count_frames doesn't work
                frame_count = reader.get_length()
            
            # Get the requested frame or last frame if out of bounds
            frame_idx = min(frame_id, frame_count - 1)
            frame = reader.get_data(frame_idx)
            reader.close()
            return Image.fromarray(frame)
        except Exception as e:
            logging.warning(f"Error reading video {video_path}: {e}")
...
def get_video_codec(video_path):
    ...
    try:
        ...
        logging.warning(f"Could not determine video codec: {e}")
        return ""
...
def convert_video_to_h264(input_path, output_path):
    ...
    try:
        subprocess.run(
            ['ffmpeg', '-i', input_path, '-c:v', 'libx264', '-c:a', 'copy', '-y', output_path],
...
def split_wav_librosa(wav_path, segments, save_dir):
    """Split audio file based on time segments"""
    ...
    for idx, (start, end) in enumerate(segments):
        ...
        out_path = os.path.join(save_dir, f"{filename}_{start:.2f}_{end:.2f}.wav")
        sf.write(out_path, segment, sr)
        logging.info(f"Saved audio segment: {out_path}")
...
def shot_detect(video_path, output_dir):
    ...
    Returns empty lists if scenedetect is not available.
    """
    try:
        ...
        scene_list = []
        for stamp in scene_manager.get_scene_list():
            start, end = stamp
            ...
            with VideoFileClip(video_path) as video_clip:
                for (start_time, end_time) in scene_list:
                    if end_time - start_time > 0.5:
                        ...
                        output_fp = os.path.join(output_dir, f'{vid_name}_{start_time:.2f}_{end_time:.2f}.mp4')
                        sub_clip.write_videofile(output_fp, logger=None)
        ...
        logging.warning("scenedetect not available, skipping scene segmentation")
        return [], [video_path]
    except Exception as e:
...
def _parse_args():
    ...
    parser.add_argument(
        ...
        help="The bucket size of the generated video. The aspect ratio of the output video will follow that of the input video."
    )
    parser.add_argument(
        "--frame_num",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The path to the DIT model checkpoint.")
    parser.add_argument(
        "--scene_seg",
        action="store_true",
        default=False,
    ...
    parser.add_argument(
        ...
        type=int,
        default=9,
        help="Driven frame length used in the mode of long video generation.")
    parser.add_argument(
        "--mode",
        type=str,
        default="clip",
...
class InfiniteTalkPipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            dit_path (`str`, *optional*, defaults to None):
                Path to DIT model checkpoint for InfiniteTalk.
            infinitetalk_dir (`str`, *optional*, defaults to None):
                Path to InfiniteTalk-specific checkpoint directory.
        """
    ...
    def generate_infinitetalk(self,
                 ...
                 extra_args=None):
        r"""
        Generates video frames from input video and text prompt using diffusion process.
...
def generate(args):
    ...
    if args.input_json:
        ...
        input_data = {
            'prompt': args.prompt,
            'cond_video': args.cond_image,  # Note: args.cond_image is actually a video path for InfiniteTalk
            'cond_audio': {},
    ...
    # Handle scene segmentation if enabled
    conds_list = []
    generated_list = []
    
    # Get video name for save directory
    video_name = input_data.get('cond_video', input_data.get('cond_image', 'video')).split('/')[-1].split('.')[0]
    args.audio_save_dir = os.path.join(args.audio_save_dir, video_name)
    os.makedirs(args.audio_save_dir, exist_ok=True)
    
    # Check for scene segmentation
    if args.scene_seg and 'cond_video' in input_data and is_video(input_data['cond_video']):
        time_list, cond_list = shot_detect(input_data['cond_video'], args.audio_save_dir)
        if len(time_list) == 0:
            conds_list.append([input_data['cond_video']])
            conds_list.append([input_data['cond_audio'].get('person1', '')])
            if 'person2' in input_data.get('cond_audio', {}):
                conds_list.append([input_data['cond_audio']['person2']])
        else:
            # Split audio by scenes
            audio1_list = split_wav_librosa(input_data['cond_audio']['person1'], time_list, args.audio_save_dir) if 'person1' in input_data.get('cond_audio', {}) else []
        ...
        conds_list.append([input_data.get('cond_video', input_data.get('cond_image'))])
        conds_list.append([input_data['cond_audio'].get('person1', '')])
        if 'person2' in input_data.get('cond_audio', {}):
            conds_list.append([input_data['cond_audio']['person2']])
    ...
    if args.scene_seg and len(conds_list[0]) > 1:
        # Process multiple scenes
        for idx, items in enumerate(zip(*conds_list)):
            logging.info(f"Processing scene {idx + 1}/{len(conds_list[0])}")
            input_clip = {}
            ...
            if len(items) > 1 and items[1]:
                ...
                audio_embedding = get_embedding(human_speech, wav2vec_feature_extractor, audio_encoder)
                emb_path = os.path.join(args.audio_save_dir, f'scene_{idx}_1.pt')
                torch.save(audio_embedding, emb_path)
                cond_audio['person1'] = emb_path
                
            if len(items) > 2 and items[2]:
                human_speech = audio_prepare_single(items[2])
                audio_embedding = get_embedding(human_speech, wav2vec_feature_extractor, audio_encoder)
                emb_path = os.path.join(args.audio_save_dir, f'scene_{idx}_2.pt')
                torch.save(audio_embedding, emb_path)
                cond_audio['person2'] = emb_path
            ...
            video = wan_i2v.generate_infinitetalk(
                ...
                seed=args.base_seed + idx,  # Vary seed per scene
        ...
        if len(generated_list) > 1:
            video = torch.cat(generated_list, dim=1)
        else:
            video = generated_list[0]
    else:
        # Single scene or no segmentation
        video = wan_i2v.generate_infinitetalk(
            input_data,
            size_buckget=args.size,
            motion_frame=args.motion_frame,
